...
foreach($stocks->children() as $item) {
		...
		$_stocks["" . $item->sku . ""] = $item->stock;

}

/*
function chkStock( $val ) { // int $val
...
foreach($xml->children() as $item) {
	...
	#$no=(int)$item->number;
	$item_node->appendChild( $dom->createElement('stock', $_stocks["" . $item->number . ""] ) );
	// $item_node->appendChild( $dom->createElement('stock', chkStock($item->number)));
